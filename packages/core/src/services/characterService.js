...
export const handleFactionChange = (character, factionId) => {
...
export const assignFreeLore = (character) => {
  const freeLoreIds = [];
...
export const handleSubfactionChange = (character, subfactionId) => {
...
export const isBreedAvailableForTribe = (subfactionId, breedId) => {
  const subfaction = gameData.subfactions.find(sf => sf.subfaction_id === subfactionId);
...
export const handleFactionChangeTransformation = (character, newFactionId) => {
...
export const calculateXPCost = (character, type, itemId, level = 1) => {
...
export const isRedundantPower = (character, treeId, level) => {
...
export const calculateReductionRefund = (character, type, itemId, level) => {
  switch (type) {
    case 'skill':
      const currentSkillLevel = character.skills[itemId] || 0;
...
export const getAvailableMerits = (character, isAdvancement = false) => {
...
export const getAvailableLores = (character, isAdvancement = false) => {
...
export const deleteCharacter = (characterId) => {
  if (window.confirm('Are you sure you want to delete this character? This cannot be undone.')) {
    // This function will need to be updated to interact with a character list passed in
    // For now, it will just return a placeholder indicating success or failure
    console.log(`Character ${characterId} deleted (placeholder).`);
    return true; // Indicate success
  }
  return false; // Indicate cancellation
};

export const advanceCharacter = (character, advancement) => {
      ...
      case 'merit':
        ...
        if (merit && merit.can_purchase_multiple === 'true') {
          if ((characterUpdate.merits[itemId] || 0) > 1) {
            ...
            const { [itemId]: removed, ...remainingMerits } = characterUpdate.merits;
            characterUpdate = {
              ...characterUpdate,
