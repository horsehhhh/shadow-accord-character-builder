      ...
      case 'pdf':
        ...
        (async () => {
          try {
            ...
            // ===== FUNDAMENTAL POWERS + ADVANTAGES / DISADVANTAGES =====
            {
              const advDisadvLines = [];

              // Fundamental powers (inherent abilities granted by faction/subfaction)
              if (character.fundamentalPowers?.length > 0) {
                character.fundamentalPowers.forEach(power => {
                  advDisadvLines.push(formatDisplayText(power));
                });
              }

              // Self-nerfs: flaws, derangements, mutations, permataint
              if (character.selfNerfs?.length > 0) {
                character.selfNerfs.forEach(nerf => {
                  if (nerf.name) advDisadvLines.push(nerf.name);
                });
              }
              ...
              // Fill AdvweaknessRow fields (try up to 8 in case PDF has extra rows)
