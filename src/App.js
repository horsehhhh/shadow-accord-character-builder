            ...
            // Structure: Source/Tree Name 1-33, each tree gets 3 powers (Power 1-105)
            {
              ...
              if (character.merits && gameData.merits) {
                const MERIT_SHORT_DESC = {
                  ...
                  moon_ties:            null, // resolved per auspice below
                };
                // Static (always-on) benefit per auspice for Moon Ties
                const MOON_TIES_STATIC = {
                  ahroun:   'Augment 1 while Frenzied',
                  galliard: 'Sense Rank',
                  philodox: 'Meditate on self',
                  ragabash: 'Reverse Mimic <Gnosis>',
                  theurge:  '+1 Gnosis: Spirit summons',
                };
                const meritEntries = Object.keys(character.merits || {})
                  .filter(meritId => meritId in MERIT_SHORT_DESC)
                  .map(meritId => {
                    const shortDesc = meritId === 'moon_ties'
                      ? (MOON_TIES_STATIC[character.auspice] || null)
                      : MERIT_SHORT_DESC[meritId];
                    return {
                      merit: gameData.merits.find(m => m.merit_id === meritId),
                      shortDesc,
                    };
                  })
                  .filter(e => e.merit && e.shortDesc);
                ...
                while (meritBatchStart < meritEntries.length && treeIndex <= 33) {
                  ...
                  let batchCount = 0;
                  for (const { merit, shortDesc } of batch) {
                    if (powerIndex <= 105) {
                      setFormField(`Power ${powerIndex}`, formatDisplayText(merit.merit_name));
                      setFormField(`Cost ${powerIndex}`, shortDesc);
                      powerIndex++;
                      batchCount++;
                    }
                  }
