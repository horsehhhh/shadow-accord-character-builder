        ...
        (async () => {
          try {
            ...
            if (character.powers && gameData.powerTrees) {
              ...
              // Add skill-granted powers after regular powers
              const skillPowers = getSkillGrantedPowers(character);
              if (skillPowers.length > 0 && treeIndex <= 33) {
                // Add "Skills" as a tree name
                setFormField(`Source/Tree Name ${treeIndex}`, 'Skills');
                console.log(`Setting Source/Tree Name ${treeIndex}: Skills`);
                
                // Add skill powers (up to 3 per "tree", can add multiple skill "trees" if needed)
                let skillPowerCount = 0;
                for (let i = 0; i < skillPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                  const skillPower = skillPowers[i];
                  const powerName = typeof skillPower === 'object' ? skillPower.name : skillPower;
                  const powerCost = typeof skillPower === 'object' ? skillPower.cost : 'Skill';
                  
                  setFormField(`Power ${powerIndex}`, powerName);
                  setFormField(`Cost ${powerIndex}`, powerCost);
                  
                  console.log(`Setting Power ${powerIndex}: "${powerName}" from Skills (${powerCost})`);
                  powerIndex++;
                  skillPowerCount++;
                }
                
                // Fill remaining slots for this skills "tree" with empty entries
                ...
                // If there are more skill powers, add another "Skills" tree
                let remainingSkillPowers = skillPowers.slice(3);
                while (remainingSkillPowers.length > 0 && treeIndex <= 33) {
                  setFormField(`Source/Tree Name ${treeIndex}`, 'Skills');
                  console.log(`Setting Source/Tree Name ${treeIndex}: Skills (continued)`);
                  
                  skillPowerCount = 0;
                  for (let i = 0; i < remainingSkillPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                    const skillPower = remainingSkillPowers[i];
                    const powerName = typeof skillPower === 'object' ? skillPower.name : skillPower;
                    const powerCost = typeof skillPower === 'object' ? skillPower.cost : 'Skill';
                  ...
                  remainingSkillPowers = remainingSkillPowers.slice(3);
                  treeIndex++;
                }
              }
            }
