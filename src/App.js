        ...
        (async () => {
          try {
            ...
            if (character.powers && gameData.powerTrees) {
              ...
              // Fundamental powers go first
              if (character.fundamentalPowers?.length > 0 && treeIndex <= 33) {
                setFormField(`Source/Tree Name ${treeIndex}`, 'Fundamentals');
                let fundPowerCount = 0;
                character.fundamentalPowers.forEach(power => {
                  if (powerIndex <= 105 && fundPowerCount < 3) {
