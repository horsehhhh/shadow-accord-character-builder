        ...
        case 1: // Subfaction Selection
          ...
          return (
            <div className="space-y-6">
              ...
              {/* DEBUG: Wraith Creation Status */}
              {newCharacter.faction === 'wraith' && (
                <div className={`${themeClasses.card} p-4 mt-5 border border-yellow-600 bg-yellow-900 bg-opacity-30`}>
                  <h4 className="text-lg font-bold text-yellow-400 mb-3">🔍 DEBUG: Wraith Requirements Status</h4>
                  <div className="space-y-2 font-mono text-sm">
                    <div className={newCharacter.innateTreeIds.length === 3 ? 'text-green-400' : 'text-red-400'}>
                      ✓ Innate Arcanoi: {newCharacter.innateTreeIds.length}/3 {newCharacter.innateTreeIds.length === 3 ? '✅' : '❌'}
                    </div>
                    <div className={newCharacter.shadowArchetype ? 'text-green-400' : 'text-red-400'}>
                      ✓ Shadow Archetype: {newCharacter.shadowArchetype || 'EMPTY'} {newCharacter.shadowArchetype ? '✅' : '❌'}
                    </div>
                    <div className={newCharacter.selectedThorn ? 'text-green-400' : 'text-red-400'}>
                      ✓ Thorn: {newCharacter.selectedThorn || 'EMPTY'} {newCharacter.selectedThorn ? '✅' : '❌'}
                    </div>
                    <div className={newCharacter.legion ? 'text-green-400' : 'text-red-400'}>
                      ✓ Legion: {newCharacter.legion || 'EMPTY'} {newCharacter.legion ? '✅' : '❌'}
                    </div>
                    <div className={newCharacter.selectedPassions.length === 2 ? 'text-green-400' : 'text-red-400'}>
                      ✓ Passions: {newCharacter.selectedPassions.length}/2 {newCharacter.selectedPassions.length === 2 ? '✅' : '❌'}
                    </div>
                  </div>
                  <div className="mt-3 p-2 bg-gray-900 rounded text-gray-300 text-xs">
                    <strong>Raw State:</strong> {JSON.stringify({legion: newCharacter.legion, shadowArchetype: newCharacter.shadowArchetype, selectedThorn: newCharacter.selectedThorn, innateTreeIds: newCharacter.innateTreeIds.length, passions: newCharacter.selectedPassions.length})}
                  </div>
                </div>
              )}
