          ...
          try {
            ...
            const getSkillGrantedPowers = (character) => {
              ...
              if (character.skills?.archery >= 2) {
                skillPowers.push({ name: 'Root', cost: 'Archery 2' }); // Pinning Shot
              }
              if (character.skills?.archery >= 3) {
                skillPowers.push({ name: 'Brutal Strike', cost: 'Archery 3' }); // Overdraw
              }
              
              // Brawl skill powers
              if (character.skills?.brawl >= 3) {
                skillPowers.push({ name: 'Daze', cost: 'Brawl 3' }); // Knockout
              }
              
              // Guidance skill powers
              if (character.skills?.guidance >= 1) {
                skillPowers.push({ name: 'Guidance', cost: 'Guidance 1' }); // Fascination (with Passion)
              }
              
              // Herbalism skill powers
              if (character.skills?.herbalism >= 3) {
                skillPowers.push({ name: 'Resist Poison', cost: 'Herbalism 3' }); // Mithridatism
              }
              
              // Holy Water skill powers (Human only)
              if (character.faction === 'human' && character.skills?.holy_water >= 2) {
                skillPowers.push({ name: 'Cleanse', cost: 'Holy Water 2' }); // Purify
              }
              if (character.faction === 'human' && character.skills?.holy_water >= 3) {
                skillPowers.push({ name: 'Sanctuary', cost: 'Holy Water 3' }); // Sanctify
              }
              
              // Medicine skill powers
              if (character.skills?.medicine >= 1) {
                skillPowers.push({ name: 'Medicine', cost: 'Medicine 1' }, { name: 'Sense Health', cost: 'Medicine 1' }); // Health Check
              }
              if (character.skills?.medicine >= 2) {
                skillPowers.push({ name: 'Detect Dead', cost: 'Medicine 2' }, { name: 'Detect Dying', cost: 'Medicine 2' }, { name: 'Detect Incapacitated', cost: 'Medicine 2' }); // First Aid
              }
              if (character.skills?.medicine >= 3) {
                skillPowers.push({ name: 'Detect Condition', cost: 'Medicine 3' }); // Diagnosis
              }
              
              // Melee skill powers
              if (character.skills?.melee >= 3) {
                skillPowers.push({ name: 'Disarm', cost: 'Melee 3' }); // Flourish
              }
              
              // Shields skill powers
              if (character.skills?.shields >= 2) {
                skillPowers.push({ name: 'Withstand', cost: 'Shields 2' }); // Glancing Blow
              }
              if (character.skills?.shields >= 3) {
                skillPowers.push({ name: 'Avoidance', cost: 'Shields 3' }); // Deflection
              }
