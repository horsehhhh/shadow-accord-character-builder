          ...
          try {
            ...
            const getSkillGrantedPowers = (character) => {
              ...
              if (character.skills?.archery >= 2) {
                skillPowers.push('Root'); // Pinning Shot
              }
              if (character.skills?.archery >= 3) {
                skillPowers.push('Brutal Strike'); // Overdraw
              }
              
              // Brawl skill powers
              if (character.skills?.brawl >= 3) {
                skillPowers.push('Daze'); // Knockout
              }
              
              // Guidance skill powers
              if (character.skills?.guidance >= 1) {
                skillPowers.push('Guidance'); // Fascination (with Passion)
              }
              
              // Herbalism skill powers
              if (character.skills?.herbalism >= 3) {
                skillPowers.push('Resist Poison'); // Mithridatism
              }
              
              // Holy Water skill powers (Human only)
              if (character.faction === 'human' && character.skills?.holy_water >= 2) {
                skillPowers.push('Cleanse'); // Purify
              }
              if (character.faction === 'human' && character.skills?.holy_water >= 3) {
                skillPowers.push('Sanctuary'); // Sanctify
              }
              
              // Medicine skill powers
              if (character.skills?.medicine >= 1) {
                skillPowers.push('Medicine', 'Sense Health'); // Health Check
              }
              if (character.skills?.medicine >= 2) {
                skillPowers.push('Detect Dead', 'Detect Dying', 'Detect Incapacitated'); // First Aid
              }
              if (character.skills?.medicine >= 3) {
                skillPowers.push('Detect Condition'); // Diagnosis
              }
              
              // Melee skill powers
              if (character.skills?.melee >= 3) {
                skillPowers.push('Disarm'); // Flourish
              }
              
              // Shields skill powers
              if (character.skills?.shields >= 2) {
                skillPowers.push('Withstand'); // Glancing Blow
              }
              if (character.skills?.shields >= 3) {
                skillPowers.push('Avoidance'); // Deflection
              }
            ...
            if (character.powers && gameData.powerTrees) {
              ...
              if (skillPowers.length > 0 && treeIndex <= 33) {
                ...
                for (let i = 0; i < skillPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                  ...
                  setFormField(`Power ${powerIndex}`, skillPower);
                  
                  // Set cost for skill powers (they're typically free/skill-based)
                  setFormField(`Cost ${powerIndex}`, 'Skill');
                  
                  console.log(`Setting Power ${powerIndex}: "${skillPower}" from Skills`);
                  powerIndex++;
                  skillPowerCount++;
                }
                ...
                while (remainingSkillPowers.length > 0 && treeIndex <= 33) {
                  ...
                  for (let i = 0; i < remainingSkillPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                    ...
                    console.log(`Setting Power ${powerIndex}: "${skillPower}" from Skills (continued)`);
                    powerIndex++;
                    skillPowerCount++;
                  }
