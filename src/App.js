  ...
  const exportCharacter = useCallback((character, format = 'json') => {
    ...
    switch (format) {
      ...
      case 'pdf-debug':
        ...
        (async () => {
          try {
            const templateResponse = await fetch('/character-sheet-template-renamed.pdf');
      ...
      case 'pdf':
        ...
        (async () => {
          try {
            ...
            const templateResponse = await fetch('/Shadow accord fixed fillable character sheet 7.24.pdf');
            const templateBytes = await templateResponse.arrayBuffer();
    ...
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }, [generateCharacterSheet, gameData.merits, gameData.powerTrees, gameData.skills, formatDisplayText]);
