...
import { 
  ...
  Trash2, X, Search, Users, Book, BookOpen,
...
import MagicItemWizard from './components/MagicItemWizard';
import MagicItemWizardClassic from './components/MagicItemWizardClassic';
import TokenWizard from './components/TokenWizard';
import TokenWizardDraft from './components/TokenWizardDraft';
...
const loadPdfFile = async (filename) => {
  try {
    console.log('🔍 PDF Load Environment Check:', {
      hasElectronAPI: !!window.electronAPI,
      electronAPIKeys: window.electronAPI ? Object.keys(window.electronAPI) : 'N/A',
      isElectron: window.electronAPI ? window.electronAPI.isElectron : 'N/A',
      hasCapacitor: typeof Capacitor !== 'undefined',
      capacitorNative: typeof Capacitor !== 'undefined' ? Capacitor.isNativePlatform() : 'N/A',
      userAgent: navigator.userAgent,
      windowElectron: typeof window !== 'undefined' ? !!window.electronAPI : 'N/A',
      allWindowProps: Object.keys(window).filter(key => key.includes('electron') || key.includes('Electron'))
    });

    // Additional debug: Check if we can detect Electron environment by other means
    const isElectronUserAgent = /electron/i.test(navigator.userAgent);
    const hasProcessVersions = typeof process !== 'undefined' && process.versions && process.versions.electron;
    
    console.log('🔍 Electron Detection Debug:', {
      userAgentElectron: isElectronUserAgent,
      processVersions: hasProcessVersions,
      processElectron: hasProcessVersions ? process.versions.electron : 'N/A'
    });

    // Check if we're running in Electron
    if (window.electronAPI && window.electronAPI.isElectron && typeof window.electronAPI.loadPdfFile === 'function') {
      console.log(`📁 Loading PDF via Electron IPC: ${filename}`);
      try {
        const arrayBuffer = await window.electronAPI.loadPdfFile(filename);
        console.log(`✅ Successfully loaded PDF via Electron: ${filename}, size:`, arrayBuffer.byteLength);
        return arrayBuffer;
      } catch (electronError) {
        console.error('❌ Electron IPC failed, falling back to web fetch:', electronError);
        // Fall through to web fetch as backup
      }
    } else {
      console.log('🚫 Electron API not available or incomplete:', {
        hasElectronAPI: !!window.electronAPI,
        electronAPIType: typeof window.electronAPI,
        isElectronValue: window.electronAPI ? window.electronAPI.isElectron : 'API missing',
        hasLoadPdfFile: window.electronAPI ? typeof window.electronAPI.loadPdfFile : 'API missing'
      });
    }
    ...
    if (typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform()) {
      ...
      const isElectronFallback = window.electronAPI && !window.electronAPI.isElectron;
      console.log(`🌐 Loading PDF via web fetch: ${filename}`, isElectronFallback ? '(Electron fallback)' : '(Web browser)');
      
      try {
        const response = await fetch(`/${filename}`);
        if (!response.ok) {
          const errorMsg = `Failed to fetch PDF: ${response.status} ${response.statusText}`;
          if (isElectronFallback) {
            throw new Error(`${errorMsg} - Electron app should use IPC, not web fetch. Check if preload.js is properly loaded.`);
          }
        ...
        if (isElectronFallback) {
          console.error('❌ Web fetch failed in Electron app. This suggests the PDF template is not accessible via file:// protocol.');
          throw new Error(`PDF template not found. In Electron apps, PDFs should be loaded via IPC. Original error: ${fetchError.message}`);
        }
        throw fetchError;
      }
    }
...
const createMobileOptimizedHandler = (handler) => {
  return async (event) => {
    ...
    let loadingModal = null; // Declare at function scope
    ...
    try {
      ...
      // For PDF exports, show a loading modal with progress info
      const isPDFExport = handler.toString().includes('pdf') || button.textContent.includes('PDF');
      
      if (isPDFExport) {
        // Create loading modal for PDF operations
        loadingModal = document.createElement('div');
        loadingModal.className = 'fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[10000]';
        loadingModal.innerHTML = `
          <div class="bg-gray-800 border border-gray-600 rounded-lg p-6 max-w-sm w-full mx-4 text-center">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
            <h3 class="text-lg font-semibold text-white mb-2">Generating PDF...</h3>
            <p class="text-gray-300 text-sm">Please wait, this may take a moment on mobile devices.</p>
            <div class="mt-4 bg-gray-700 rounded-full h-2">
              <div class="bg-blue-500 h-2 rounded-full transition-all duration-1000" style="width: 0%" id="pdf-progress"></div>
            </div>
          </div>
        `;
        ...
        const progressBar = loadingModal.querySelector('#pdf-progress');
        let progress = 0;
      ...
      if (loadingModal) {
        ...
        setTimeout(() => {
          loadingModal.querySelector('h3').textContent = 'PDF Generated!';
          loadingModal.querySelector('p').textContent = 'Your file should start downloading now.';
      ...
      // Show success feedback briefly on button
      ...
      if (loadingModal && loadingModal.parentNode) {
        loadingModal.querySelector('h3').textContent = 'Export Failed';
        loadingModal.querySelector('p').textContent = error.message || 'An error occurred during export.';
        loadingModal.querySelector('.animate-spin').className = 'h-12 w-12 text-red-500 mx-auto mb-4';
...
const ShadowAccordComplete = () => {
  ...
  // Helper function to format text by replacing underscores with spaces and applying title case
  const formatDisplayText = useCallback((text) => {
    if (!text) return text;
    // Ensure text is a string, replace underscores with spaces, then apply title case
    return String(text)
      .replace(/_/g, ' ')
  ...
  const changelog = [
    ...
    },
    {
      ...
      date: '2026-05-12',
      changes: [
        '📖 Test Oath renamed to Test Blood Bond; call and description updated to reference Blood Bond (p. 149)'
      ]
    },
    {
      version: '0.5.1',
    ...
    },
    {
      ...
      date: '2026-05-06',
      changes: [
        '📄 PDF Fundamental Powers (Issue #9) - Fundamental powers (Regeneration, Sense Spirit, Frail, etc.) now export to the Advantages/Weaknesses rows on the character sheet',
        '📄 PDF Flaws & Derangements (Issue #9) - Self-nerfs (flaws, derangements, mutations, permataint) now export to the Advantages/Weaknesses rows below fundamental powers',
        '📄 PDF Adv/Disadv ordering - Fundamental powers list first, then flaws/derangements/mutations, then any legacy free-text advantages'
      ]
    },
    {
      version: '0.4.2',
      date: '2026-05-05',
      changes: [
        '🔄 Faction Change Dormant Tags - Powers from a character\'s original faction (e.g. vampire-turned-wraith) are now marked DORMANT in the advancement panel with amber styling',
        '📄 PDF Export Dormant Label - Dormant trees print as "Tree Name (Dormant)" on the exported character sheet',
        '🐛 Fix TDZ crash - Removed gameData from useEffect dependency array where it was evaluated before initialization'
      ]
    },
    {
      version: '0.4.1',
...
pleasure,Pleasure,Joy|excitement|comfort`
  ...
  const createBlankCharacter = () => ({
    ...
    deedname: '', // For shifter deed name
    sire: '', // For vampire sire name
    kinfolkTribe: null, // For Kinfolk merit tribe selection
    ...
    teachingsReceived: [],
    teachingsGiven: [],
    selfNerfs: [], // For storing flaws, derangements, mutations, and permataint
  ...
  const assignFreeLore = (character) => {
    ...
    if (character.subfaction) {
      ...
      // Kinfolk merit: gives Shifter Lore + tribal lore for the selected tribe
      if (character.merits?.kinfolk && character.kinfolkTribe) {
        freeLoreIds.push('general_shifter');
        const feraLoreId = `${character.kinfolkTribe}_lore`;
        const feraLore = gameData.lores.find(lore => lore.lore_id === feraLoreId);
        if (feraLore) {
          freeLoreIds.push(feraLoreId);
        } else {
          const tribalLoreId = `tribe_${character.kinfolkTribe}`;
          const tribalLore = gameData.lores.find(lore => lore.lore_id === tribalLoreId);
          if (tribalLore) {
            freeLoreIds.push(tribalLoreId);
          }
  ...
  const handleSubfactionChange = (character, subfactionId) => {
    ...
    if (character.faction === 'wraith' || 
      ...
      // For ghouls, add the three innate vampire trees (Celerity, Fortitude, Potence) with one free dot in Potence
      if (subfactionId === 'ghoul') {
        innateTreeIds = ['celerity', 'fortitude', 'potence'];
        freeFirstDotPowers = ['potence'];
        // Auto-assign Potence 1 for free to ghouls
        character.powers = {
          ...character.powers,
          potence: { 1: true }
        };
  ...
  const calculateXPCost = useCallback((character, type, itemId, level = 1) => {
    ...
    if (type === 'merit') {
      ...
      const currentMeritCount = Object.entries(character.merits || {}).reduce((total, [meritId, quantity]) => {
        ...
        if (!meritData || meritData.special_notes?.includes('Does not increase cost of future merits')) {
          return total; // Skip merits flagged as non-progressive
        }
        if (meritData.can_purchase_multiple === 'true') {
      ...
      // For humans, first merit is free; subsequent merits cost 3*slot (3, 6, 9...)
      // For non-humans, all merits cost 3*(slot+1) — 3, 6, 9...
      // Humans: free, 3, 6, 9... → cost = 3 * currentMeritCount (currentMeritCount already includes free slot)
      // Non-humans: 3, 6, 9... → cost = 3 * (currentMeritCount + 1)
      if (character.faction === 'human') {
        if (currentMeritCount === 0) return 0; // first merit is free
        return 3 * currentMeritCount; // 2nd merit=3, 3rd=6, 4th=9...
      }
      return 3 * (currentMeritCount + 1); // 1st merit=3, 2nd=6, 3rd=9...
    }
  ...
  const getScorchedBy = (character) => {
    ...
    if (faction === 'wraith') {
      return ['<Dark>', '<Fire>(realm)', '<Blood>(realm)'];
    }
    if (faction === 'human') {
      return ['<Fire>', '<Blood>', '<Dark>'];
    }
    return [];
  };

  const getClanCurse = (subfaction) => {
    const CLAN_CURSES = {
      assamite:        'Drinking blood from a Vampire afflicts you with "Craving: <Target>\'s Blood".',
      baali:           'You are <Holy> Scorched. Holy Water causes 1 Agg damage if consumed. Holy Ground/Symbol causes <Holy> Exposure damage.',
      brujah:          'Cannot resist Frenzy through any means.',
      ...
      malkavian:       'Must choose at least one Derangement at character creation.',
      nosferatu:       'Have mottled or disfigured skin as a makeup requirement.',
      ravnos:          'When encountering a riddle or puzzle, must engage with it for at least 60 seconds or until solved.',
      salubri_healer:  'Feeding on an unwilling Human causes 1 damage per Health drained. Must wear a third eye on forehead.',
      salubri_warrior: 'Feeding on an unwilling Human causes 1 damage per Health drained. Must wear a third eye on forehead.',
      toreador:        'Each time you see something beautiful, stare at it for 10 seconds.',
  ...
  const calculateReductionRefund = useCallback((character, type, itemId, level) => {
    switch (type) {
      ...
      case 'merit': {
        ...
        // Compute total progressive merit instances (all non-flat merits) including this one
        const totalProgressiveInstances = Object.entries(character.merits || {}).reduce((total, [mid, qty]) => {
          const md = gameData.merits.find(m => m.merit_id === mid);
          if (!md || md.special_notes?.includes('Does not increase cost of future merits')) return total;
          return total + (md.can_purchase_multiple === 'true' ? qty : 1);
        }, 0);
        if (character.faction === 'human') {
          // Human slot cost: slot 1 = free, slot N = 3*(N-1)
          if (totalProgressiveInstances <= 1) return 0;
          return 3 * (totalProgressiveInstances - 1);
        }
        // Non-human slot cost: slot N = 3*N
        return 3 * totalProgressiveInstances;
      }
      case 'energy':
  ...
  const canLearnPower = (character, treeId, level) => {
    ...
    // Ghouls learn any vampire power sequentially (like other vampires)
    if (character.subfaction === 'ghoul') {
      // Ghouls learn sequentially like other vampires
  ...
  // Returns { met: bool, label: string|null } for merit prerequisites
  const getMeritPrereqStatus = useCallback((character, meritId) => {
    if (meritId !== 'oracle') return { met: true, label: null };

    const hasNonDormantPower = (treeId, level) => {
      if (!character.powers?.[treeId]?.[level]) return false;
      const tree = gameData.powerTrees.find(t => t.tree_id === treeId);
      return tree && !isTreeDormant(character, tree);
    };

    const theurgeTree = gameData.powerTrees.find(t => t.tree_id === 'theurge');
    const hasTheurge = !!(character.innateTreeIds?.includes('theurge') &&
      theurgeTree && !isTreeDormant(character, theurgeTree));
    const hasDementation1 = hasNonDormantPower('dementation', 1);
    const hasFatalism1 = hasNonDormantPower('fatalism', 1);
    const hasGuidance3 = (character.skills?.guidance || 0) >= 3; // Guidance is a skill, never dormant

    const met = hasTheurge || hasDementation1 || hasFatalism1 || hasGuidance3;
    return {
      met,
      label: met ? null : 'Requires: Theurge Auspice, Dementation 1, Fatalism 1, or Guidance 3'
    };
  }, [gameData.powerTrees, isTreeDormant]);
...
`;
  ...
  const createTestCharacter = useCallback(async () => {
    try {
      ...
      const testCharacter = {
        ...
        merits: [
          { meritId: 'ambidextrous', name: 'Ambidextrous', cost: 1 },
          { meritId: 'acute_hearing', name: 'Acute Hearing', cost: 1 }
        ],
        lores: {
          'vampire_lore': 3,
          'sect_lore_brujah': 2
        },
        notes: 'This is a test character created for debugging PDF export and other features.',
        created: new Date().toISOString(),
        id: `test_${Date.now()}`,
        ...
        shadowArchetype: '',
        selectedThorn: '',
        mixedSubfaction: null,
        claimedStatus: null,
        selectedFomoriTree: null,
        firstMeritFree: false
      };
      
      // Add to characters array using cloud sync if available
      ...
      try {
        ...
        const errorDetails = {
          ...
          type: cloudError.name || 'Unknown',
          status: cloudError.response?.status || 'N/A',
          isVersionError: !!cloudError.isVersionError,
          hasResponse: !!cloudError.response
        };
        
        throw new Error(`Character creation failed!\n\nError: ${cloudError.message}\nType: ${errorDetails.type}\nStatus: ${errorDetails.status}\nVersion Error: ${errorDetails.isVersionError}`);
      }
      
    } catch (error) {
      console.error('❌ Error creating test character:', error);
      alert('Error creating test character: ' + error.message);
    }
  ...
  const exportCharacter = useCallback((character, format = 'json') => {
    ...
    switch (format) {
      ...
      case 'pdf-debug':
        ...
        return;
      case 'pdf':
        // Handle PDF export separately - call it async with timeout
        console.log('🔄 Starting PDF export...');
        (async () => {
          try {
            console.log('📄 PDF export: Starting async processing...');
            console.log('🔍 PDF export: About to create timeout promise...');
            
            // Add timeout for mobile devices - PDF generation can be slow
            const timeoutPromise = new Promise((_, reject) => {
              setTimeout(() => reject(new Error('PDF generation timed out. This may be due to device limitations.')), 30000); // 30 second timeout
            });
            
            console.log('🔍 PDF export: About to create PDF promise...');
            const pdfPromise = (async () => {
              console.log('📂 PDF export: About to call loadPdfFile...');
              
              // For web browsers, test if the file exists first. Skip for Electron/Capacitor.
              const isElectronApp = window.electronAPI?.isElectron && typeof window.electronAPI.loadPdfFile === 'function';
              const isCapacitorApp = typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform();
              
              if (!isElectronApp && !isCapacitorApp) {
                try {
                  console.log('🔍 PDF export: Testing file existence via web fetch...');
                  const testResponse = await fetch('/Shadow accord fixed fillable character sheet 7.24.pdf');
                  console.log('🔍 PDF export: Test fetch response:', testResponse.status, testResponse.statusText);
                  if (!testResponse.ok) {
                    throw new Error(`PDF template not found: ${testResponse.status} ${testResponse.statusText}`);
                  }
                } catch (testError) {
                  console.error('❌ PDF export: File existence test failed:', testError);
                  throw testError;
                }
              } else {
                console.log('🔍 PDF export: Skipping file existence test (Electron/Capacitor detected)');
              }
              
              console.log('📂 PDF export: Loading template file...');
              // Load the template PDF
              const templateBytes = await loadPdfFile('Shadow accord fixed fillable character sheet 7.24.pdf');
              console.log('✅ PDF export: Template loaded successfully, size:', templateBytes.byteLength);
              
              console.log('📋 PDF export: Creating PDF document...');
              // Load the PDF document
              const pdfDoc = await PDFDocument.load(templateBytes);
              const form = pdfDoc.getForm();
              console.log('✅ PDF export: PDF document created, form fields:', form.getFields().length);
            ...
            // Helper function to get skill-granted powers
            const getSkillGrantedPowers = (character) => {
              const skillPowers = [];
              
              // Archery skill powers
              if (character.skills?.archery >= 2) {
                skillPowers.push({ name: 'Root', cost: '1/Day' }); // Pinning Shot
              }
              if (character.skills?.archery >= 3) {
                skillPowers.push({ name: 'Brutal Strike', cost: '1/Day' }); // Overdraw
              }
              
              // Brawl skill powers
              if (character.skills?.brawl >= 3) {
                skillPowers.push({ name: 'Daze', cost: '1/Day' }); // Knockout
              }
              
              // Guidance skill powers
              if (character.skills?.guidance >= 1) {
                skillPowers.push({ name: 'Guidance', cost: '1/Day' }); // Fascination (with Passion)
              }
              
              // Herbalism skill powers
              if (character.skills?.herbalism >= 3) {
                skillPowers.push({ name: 'Resist Poison', cost: '1/Day' }); // Mithridatism
              }
              
              // Holy Water skill powers (Human only)
              if (character.faction === 'human' && character.skills?.holy_water >= 2) {
                skillPowers.push({ name: 'Cleanse', cost: '1/Day' }); // Purify
              }
              if (character.faction === 'human' && character.skills?.holy_water >= 3) {
                skillPowers.push({ name: 'Sanctuary', cost: '1/Day' }); // Sanctify
              }
              
              // Medicine skill powers - only show highest level medicine power
              const medicineLevel = character.skills?.medicine || 0;
              if (medicineLevel >= 3) {
                // Medicine 3: Medicine 6 + all detect powers + Sense Health
                skillPowers.push(
                  { name: 'Medicine 6', cost: 'None' },
                  { name: 'Sense Health', cost: 'None' },
                  { name: 'Detect Dead', cost: 'None' },
                  { name: 'Detect Dying', cost: 'None' },
                  { name: 'Detect Incapacitated', cost: 'None' },
                  { name: 'Detect Condition', cost: 'None' }
                );
              } else if (medicineLevel >= 2) {
                // Medicine 2: Medicine 4 + first aid detect powers + Sense Health
                skillPowers.push(
                  { name: 'Medicine 4', cost: 'None' },
                  { name: 'Sense Health', cost: 'None' },
                  { name: 'Detect Dead', cost: 'None' },
                  { name: 'Detect Dying', cost: 'None' },
                  { name: 'Detect Incapacitated', cost: 'None' }
                );
              } else if (medicineLevel >= 1) {
                // Medicine 1: Medicine 2 + Sense Health
                skillPowers.push(
                  { name: 'Medicine 2', cost: 'None' },
                  { name: 'Sense Health', cost: 'None' }
                );
              }
              
              // Melee skill powers
              if (character.skills?.melee >= 3) {
                skillPowers.push({ name: 'Disarm', cost: '1/Day' }); // Flourish
              }
              
              // Shields skill powers
              if (character.skills?.shields >= 2) {
                skillPowers.push({ name: 'Withstand', cost: '1/Day' }); // Glancing Blow
              }
              if (character.skills?.shields >= 3) {
                skillPowers.push({ name: 'Avoidance', cost: '1/Day' }); // Deflection
              }
            ...
            const getPowerCost = (powerName) => {
              ...
              const costs = powers.map(power => {
                ...
                const noCostPowers = [
                  ...
                  'Detect Condition', 'Detect Dead', 'Detect Dying', 'Detect Incapacitated',
                  ...
                  'Temporary Angst', 'Tentacles', 'Test Faction', 'Test Blood Bond', 'Test Vitae',
                  'Totemic Form', 'Toughness', 'Umbra Drain', 'Umbra Sight', 'Vengeance of Samiel',
                  'Venom Blood', 'Venomous Bite', 'Visions', 'War Form', 'Wither'
                ];
                ...
                const oneEnergyCost = [
                  ...
                  'Sense Demon', 'Taunt', 'Terror', 'Test Generation', 'True Form', 'Umbra Strike', 'Weaponry', 'Withstand',
                ...
                const twoEnergyCost = [
                  ...
                  'Dreamshape', 'Fire 4', 'Majesty', 'Mass Taunt', 'Paralyze', 'Venom'
                ];
            ...
            if (character.faction === 'shifter') {
              ...
              setFormField('Subfaction1', formatDisplayText(subfactions[0] || ''));
              setFormField('Subfaction2', formatDisplayText(subfactions[1] || ''));
              setFormField('Subfaction3', formatDisplayText(subfactions[2] || ''));
            }
            
            // ===== ADVANTAGES / DISADVANTAGES (flaws and permataint only) =====
            {
              const advDisadvLines = [];

              // Flaws and permataint from selfNerfs
              if (character.selfNerfs?.length > 0) {
                character.selfNerfs.forEach(nerf => {
                  if ((nerf.type === 'flaw' || nerf.type === 'permataint') && nerf.name) {
                    advDisadvLines.push(nerf.name);
                  }
                });
              }

              // Legacy free-text advantages field (fallback)
              if (character.advantages) {
                character.advantages.split('\n').forEach(line => {
                  if (line.trim()) advDisadvLines.push(line.trim());
                });
              }

              advDisadvLines.slice(0, 8).forEach((line, index) => {
            ...
            setFormField('Gen/Rank', formatDisplayText(genRankValue));
            
            // Fill Passion field based on faction
            if (character.faction === 'wraith' && character.selectedPassions && character.selectedPassions.length > 0) {
            ...
            if (character.faction === 'wraith' && character.shadowArchetype) {
              const shadowName = gameData.shadowArchetypes?.find(a => a.archetype_id === character.shadowArchetype)?.archetype_name || character.shadowArchetype;
              setFormField('Shadow/Deed Name/Sire1', 'Shadow');
              setFormField('Shadow/Deed Name/Sire2', shadowName);
            } else if (character.faction === 'shifter') {
              setFormField('Shadow/Deed Name/Sire1', 'Deedname');
              setFormField('Shadow/Deed Name/Sire2', character.deedname || '');
            } else if (character.faction === 'vampire') {
              setFormField('Shadow/Deed Name/Sire1', 'Sire');
            ...
            const virtueTypeNames = { vampire: 'Road', shifter: 'Rage', human: 'Humanity', wraith: 'Angst' };
            setFormField('Virtue', virtueTypeNames[character.faction] || '');
            setFormField('Devoured', character.amaranthCount ? String(character.amaranthCount) : '');
            ...
            if (character.powers && gameData.powerTrees) {
              ...
              learnedTrees.slice(0, 23).forEach((treeId, index) => {
                ...
                if (tree) {
                  ...
                  const dormantSuffix = isTreeDormant(character, tree) ? ' (Dormant)' : '';
                  console.log(`Setting learned tree (first half) ${learnedFieldIndex}: ${tree.tree_name} → ${treeFieldName}`);
                  setFormField(treeFieldName, formatDisplayText(tree.tree_name) + dormantSuffix);
              ...
              learnedTrees.slice(23, 45).forEach((treeId, index) => {
                ...
                if (tree) {
                  ...
                  setFormField(rowFieldName, formatDisplayText(tree.tree_name) + dormantSuffix);
            ...
            // Structure: Source/Tree Name 1-33, each tree gets 3 powers (Power 1-105)
            {
              ...
              // Fundamental powers go first, always — even if character has no learned trees
              if (character.fundamentalPowers?.length > 0 && gameData.powerTrees) {
                let remainingFundamentals = [...character.fundamentalPowers];
                let isFirstFundRow = true;
                while (remainingFundamentals.length > 0 && treeIndex <= 33) {
                  setFormField(`Source/Tree Name ${treeIndex}`, isFirstFundRow ? 'Fundamentals' : 'Fundamentals (cont.)');
                  let fundPowerCount = 0;
                  const batch = remainingFundamentals.splice(0, 3);
                  ...
                  while (fundPowerCount < 3 && powerIndex <= 105) {
                    setFormField(`Power ${powerIndex}`, '');
                    setFormField(`Cost ${powerIndex}`, '');
                    powerIndex++;
                    fundPowerCount++;
                  }
                  treeIndex++;
                  isFirstFundRow = false;
                }
              }
              ...
              // Add skill-granted powers after regular powers, grouped by skill type
              const allSkillPowers = getSkillGrantedPowers(character);
              if (allSkillPowers.length > 0 && treeIndex <= 33) {
                // Group powers by skill type
                const skillGroups = {};
                allSkillPowers.forEach(power => {
                  const powerName = typeof power === 'object' ? power.name : power;
                  const powerCost = typeof power === 'object' ? power.cost : 'Skill';
                  
                  // Determine skill type from power cost or name
                  let skillType = 'Skills';
                  if (powerCost.includes('Archery')) skillType = 'Archery';
                  else if (powerCost.includes('Brawl')) skillType = 'Brawl';
                  else if (powerCost.includes('Guidance')) skillType = 'Guidance';
                  else if (powerCost.includes('Herbalism')) skillType = 'Herbalism';
                  else if (powerCost.includes('Holy Water')) skillType = 'Holy Water';
                  else if (powerName.includes('Medicine') || powerName.includes('Detect') || powerName.includes('Sense Health')) skillType = 'Medicine';
                  else if (powerCost.includes('Melee')) skillType = 'Melee';
                  else if (powerCost.includes('Shields')) skillType = 'Shields';
                  else if (powerCost === '1/Day') {
                    // Fallback: try to determine from character's skills
                    if (character.skills?.archery >= 2 && (powerName === 'Root' || powerName === 'Brutal Strike')) skillType = 'Archery';
                    else if (character.skills?.brawl >= 3 && powerName === 'Daze') skillType = 'Brawl';
                    else if (character.skills?.guidance >= 1 && powerName === 'Guidance') skillType = 'Guidance';
                    else if (character.skills?.herbalism >= 3 && powerName === 'Resist Poison') skillType = 'Herbalism';
                    else if (character.faction === 'human' && character.skills?.holy_water >= 2 && (powerName === 'Cleanse' || powerName === 'Sanctuary')) skillType = 'Holy Water';
                    else if (character.skills?.melee >= 3 && powerName === 'Disarm') skillType = 'Melee';
                    else if (character.skills?.shields >= 2 && (powerName === 'Withstand' || powerName === 'Avoidance')) skillType = 'Shields';
                  }
                  
                  if (!skillGroups[skillType]) {
                    skillGroups[skillType] = [];
                  }
                  skillGroups[skillType].push({ name: powerName, cost: powerCost });
                });
                
                // Add each skill group as a separate "tree"
                Object.keys(skillGroups).forEach(skillType => {
                  if (treeIndex <= 33) {
                    const skillPowers = skillGroups[skillType];
                    setFormField(`Source/Tree Name ${treeIndex}`, skillType);
                    console.log(`Setting Source/Tree Name ${treeIndex}: ${skillType}`);
                    
                    // Add up to 3 powers for this skill type
                    let skillPowerCount = 0;
                    for (let i = 0; i < skillPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                      const skillPower = skillPowers[i];
                      
                      setFormField(`Power ${powerIndex}`, skillPower.name);
                      setFormField(`Cost ${powerIndex}`, skillPower.cost);
                      
                      console.log(`Setting Power ${powerIndex}: "${skillPower.name}" from ${skillType} (${skillPower.cost})`);
                      powerIndex++;
                      skillPowerCount++;
                    }
                    
                    // Fill remaining slots for this skill "tree" with empty entries
                    while (skillPowerCount < 3 && powerIndex <= 105) {
                      setFormField(`Power ${powerIndex}`, '');
                      setFormField(`Cost ${powerIndex}`, '');
                      powerIndex++;
                      skillPowerCount++;
                    ...
                    // If this skill type has more than 3 powers, add additional trees
                    let remainingPowers = skillPowers.slice(3);
                    while (remainingPowers.length > 0 && treeIndex <= 33) {
                      setFormField(`Source/Tree Name ${treeIndex}`, `${skillType} (cont.)`);
                      console.log(`Setting Source/Tree Name ${treeIndex}: ${skillType} (cont.)`);
                      
                      skillPowerCount = 0;
                      for (let i = 0; i < remainingPowers.length && powerIndex <= 105 && skillPowerCount < 3; i++) {
                        const skillPower = remainingPowers[i];
                      ...
                      remainingPowers = remainingPowers.slice(3);
                      treeIndex++;
                    }
                  }
                });
              }
              } // end if (character.powers && gameData.powerTrees)

              // Add merit-granted abilities as a section in the powers list (only merits with active powers)
              // Power field holds "Name - Desc" (larger); Cost field intentionally left blank (smallest field)
              if (character.merits && gameData.merits) {
                const MERIT_SHORT_DESC = {
                  nimble:               'Avoidance 1/Day',
                  hardy:                'Resilience 1/Day',
                  enhanced_blood_buff:  'Buff 3 Energy',
                  escape_artist:        'Escape 60 sec',
                  hypnotist:            'Hypnotism 60 sec',
                  steel_trap:           'Alert: Forgetful Mind',
                  strong_will:          'Mental 5 min',
                  umbral_affinity:      'Step Sideways 30 sec',
                };
                // Moon Ties: static (always-on) + phase benefit per auspice
                const MOON_TIES_BENEFITS = {
                  ahroun:   { static: 'Augment 1 while Frenzied',    phase: 'Phase: Augment 1' },
                  galliard: { static: 'Sense Rank',                   phase: 'Phase: Mass Song of Rage free' },
                  philodox: { static: 'Meditate on self',             phase: 'Phase: Unresistable Hypno free' },
                  ragabash: { static: 'Reverse Mimic <Gnosis>',       phase: 'Phase: Mass Hallucinate free' },
                  theurge:  { static: '+1 Gnosis: Spirit summons',    phase: 'Phase: Node Gnosis x2' },
                };
                const meritEntries = Object.keys(character.merits || {})
                  .flatMap(meritId => {
                    const merit = gameData.merits.find(m => m.merit_id === meritId);
                    if (!merit) return [];
                    if (meritId === 'moon_ties') {
                      const benefits = MOON_TIES_BENEFITS[character.auspice];
                      if (!benefits) return [];
                      return [
                        { label: `Moon Ties - ${benefits.static}` },
                        { label: benefits.phase },
                      ];
                    }
                    if (!MERIT_SHORT_DESC[meritId]) return [];
                    return [{ label: `${formatDisplayText(merit.merit_name)} - ${MERIT_SHORT_DESC[meritId]}` }];
                  });

                let meritBatchStart = 0;
                let isFirstMeritRow = true;
                while (meritBatchStart < meritEntries.length && treeIndex <= 33) {
                  setFormField(`Source/Tree Name ${treeIndex}`, isFirstMeritRow ? 'Merits' : 'Merits (cont.)');
                  const batch = meritEntries.slice(meritBatchStart, meritBatchStart + 3);
                  let batchCount = 0;
                  for (const { label } of batch) {
                    if (powerIndex <= 105) {
                      setFormField(`Power ${powerIndex}`, label);
                      setFormField(`Cost ${powerIndex}`, '');
                      powerIndex++;
                      batchCount++;
                    }
                  }
                  while (batchCount < 3 && powerIndex <= 105) {
                    setFormField(`Power ${powerIndex}`, '');
                    setFormField(`Cost ${powerIndex}`, '');
                    powerIndex++;
                    batchCount++;
                ...
                if (index < loreFields.length) {
                  ...
                  if (typeof lore === 'string') {
                    ...
                    if (!loreName && lore.lore_id) {
                      loreName = formatDisplayText(lore.lore_id);
                    }
                  }
            ...
            const traitLines = characterTrait === 'Unliving'
              ...
              : [`Trait: Living`];
            const scorches = getScorchedBy(character);
            const scorchLines = scorches.length ? [`Scorched: ${scorches.join(', ')}`] : [];
            const clanCurse = getClanCurse(character.subfaction);
            const curseLines = clanCurse ? [`Clan Curse: ${clanCurse}`] : [];
            const noteLines = character.notes ? character.notes.split('\n') : [];
            const allNoteLines = [...traitLines, ...scorchLines, ...curseLines, ...noteLines];
            ...
            const pdfBytes = await pdfDoc.save();
            console.log('✅ PDF export: PDF saved, size:', pdfBytes.length);
            ...
            const isActuallyElectron = window.electronAPI?.isElectron && typeof window.electronAPI.loadPdfFile === 'function';
            errorMessage += '\n\nRunning in: ' + (isActuallyElectron ? 'Electron app' : 'Web browser');
    ...
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }, [generateCharacterSheet, gameData.merits, gameData.powerTrees, gameData.skills, gameData.lores, gameData.passions, gameData.shadowArchetypes, formatDisplayText, currentVersion, isTreeDormant]);

  // =====================
  // AUTO-SAVE SYSTEM (Settings only - characters managed by cloud)
  // =====================
  useEffect(() => {
    if (autoSave) {
      const saveData = {
  ...
  }, [darkMode, accessibility, autoSave]);

  // Load saved settings on startup (characters are loaded by useCharacters hook)
  useEffect(() => {
    const savedData = localStorage.getItem('shadowAccordPhase8');
    if (savedData) {
      try {
  ...
  const filteredAndSortedCharacters = useMemo(() => {
    if (!characters || !Array.isArray(characters)) {
      console.log('🔍 Character filtering: No characters array', { characters: typeof characters, isArray: Array.isArray(characters) });
      return [];
    }
    
    console.log('🔍 Character filtering input:', {
      totalCharacters: characters.length,
      searchQuery,
      filterFaction,
      sortBy,
      characterNames: characters.map(c => c?.name || 'unnamed')
    });
    
    let filtered = characters.filter(char => {
      // Defensive programming - ensure char has required properties
      if (!char || typeof char !== 'object') {
        console.log('🔍 Filtering out invalid character:', char);
        return false;
      }
      
      const matchesSearch = !searchQuery || 
        (char.name && char.name.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (char.player && char.player.toLowerCase().includes(searchQuery.toLowerCase()));
      
      const matchesFaction = !filterFaction || char.faction === filterFaction;
      
      const shouldInclude = matchesSearch && matchesFaction;
      
      if (!shouldInclude) {
        console.log('🔍 Character filtered out:', {
          name: char.name,
          matchesSearch,
          matchesFaction,
          searchQuery,
    ...
    console.log('🔍 Character filtering result:', {
      inputCount: characters.length,
      filteredCount: filtered.length,
      filteredNames: filtered.map(c => c?.name || 'unnamed')
    });
    ...
    <div className={`min-h-screen ${themeClasses.base}`}>
      <div className="w-full max-w-4xl mx-auto px-2 py-4 sm:px-4 sm:py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-5">
          ...
          <button
            onClick={() => setCurrentMode('rules')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <BookOpen className="w-8 h-8 text-yellow-400 mx-auto mb-2 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-bold mb-1">Rules &amp; Rituals</h3>
            <p className="text-sm text-gray-400">Rulebook and ritual documents</p>
          </button>
          ...
          <button
            onClick={() => setCurrentMode('item-wizard')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <span className="text-3xl block text-center mb-2 group-hover:scale-110 transition-transform">🔒</span>
            <h3 className="text-lg font-bold mb-1">Item Wizard</h3>
            <p className="text-sm text-gray-400">2026 draft rules</p>
          </button>

          <button
            onClick={() => setCurrentMode('item-wizard-classic')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <span className="text-3xl block text-center mb-2 group-hover:scale-110 transition-transform">🔒</span>
            <h3 className="text-lg font-bold mb-1">Item Wizard (2025)</h3>
            <p className="text-sm text-gray-400">Current rules</p>
          </button>

          <button
            onClick={() => setCurrentMode('token-wizard')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <span className="text-3xl block text-center mb-2 group-hover:scale-110 transition-transform">🔒</span>
            <h3 className="text-lg font-bold mb-1">Token Rewards</h3>
            <p className="text-sm text-gray-400">Current rules</p>
          </button>

          <button
            onClick={() => setCurrentMode('token-wizard-draft')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <span className="text-3xl block text-center mb-2 group-hover:scale-110 transition-transform">🔒</span>
            <h3 className="text-lg font-bold mb-1">Token Rewards (2026)</h3>
            <p className="text-sm text-gray-400">2026 draft rules</p>
          </button>

          <button
            onClick={() => setCurrentMode('npc-creator')}
            className={`${themeClasses.card} p-5 hover:shadow-lg transition-all group cursor-pointer`}
          >
            <span className="text-3xl block text-center mb-2 group-hover:scale-110 transition-transform">👤</span>
            <h3 className="text-lg font-bold mb-1">NPC Creator</h3>
            <p className="text-sm text-gray-400">Build NPC reference cards</p>
          </button>
              ...
              <div className="grid md:grid-cols-2 gap-2">
                ...
                {newCharacter.faction === 'vampire' && (
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium mb-1">Sire</label>
                    <input
                      type="text"
                      value={newCharacter.sire || ''}
                      onChange={(e) => setNewCharacter({...newCharacter, sire: e.target.value})}
                      className={themeClasses.input + " py-1.5"}
                ...
                {newCharacter.faction === 'shifter' && (
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium mb-1">Deed Name</label>
                    <input
                      type="text"
                      value={newCharacter.deedname || ''}
                      onChange={(e) => setNewCharacter({...newCharacter, deedname: e.target.value})}
                      className={themeClasses.input + " py-1.5"}
              ...
              {newCharacter.faction !== 'wraith' && availableSubfactions.length > 0 && (
                <div className="grid md:grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    {newCharacter.faction === 'vampire' && getClanCurse(subfaction.subfaction_id) && (
                      <div className="mt-2 p-2 bg-red-900 bg-opacity-20 rounded border border-red-800">
                        <p className="text-xs text-red-300">
                          <span className="font-semibold">Clan Curse:</span> {getClanCurse(subfaction.subfaction_id)}
                        </p>
                      </div>
                    )}
          ...
          return (
            <div className="space-y-6">
              ...
              <div className={factionChangeCreationMode ? 'space-y-6' : 'grid md:grid-cols-2 gap-5'}>
                ...
                {/* Powers Section */}
                {/* Ghoul automatic Potence 1 assignment */}
                {newCharacter.faction === 'human' && newCharacter.subfaction === 'ghoul' && (
                  <div className={`${themeClasses.card} p-3 mb-3`}>
                    <h4 className="font-bold mb-2">✅ Innate Powers (Automatic)</h4>
                    <p className="text-gray-400 text-sm mb-3">
                      As a ghoul, you automatically receive <strong>Potence 1 for free</strong>. This power is already assigned and doesn't count against any advancement limits. You have three innate vampire power trees (Celerity, Fortitude, and Potence) that you can advance during play using XP.
                    </p>
                    <div className="p-3 rounded-lg border-2 border-green-500 bg-green-500 bg-opacity-20">
                      <div className="flex items-center justify-between mb-2">
                        <h5 className="font-bold text-lg">Potence</h5>
                        <div className="w-6 h-6 bg-green-500 rounded-full flex items-center justify-center">
                          <span className="text-white text-sm">✓</span>
                        </div>
                      </div>
                      <div className="p-2 rounded border border-green-400 bg-green-400 bg-opacity-10">
                        <div className="flex items-center mb-1">
                          <div className="w-4 h-4 rounded-full mr-2 bg-green-500" />
                          <span className="font-medium text-sm">Level 1 - FREE</span>
                        </div>
                        <div className="text-xs text-gray-300">Shatter</div>
                        <p className="text-xs text-green-300 mt-1 italic">Automatically granted - your free ghoul power</p>
                      </div>
                    </div>
                  </div>
                )}
                
                {/* Skip innate powers UI for ghouls—they get Potence 1 automatically */}
                {!(newCharacter.faction === 'human' && newCharacter.subfaction === 'ghoul') && (
                <div className={`${themeClasses.card} p-3`}>
                  <h4 className="font-bold mb-2">
                    Innate Powers ({newCharacter.faction === 'human' && newCharacter.subfaction === 'kinfolk' ? '1 dot' : 
                                   ...
                                   newCharacter.faction === 'human' && newCharacter.subfaction === 'sorcerer' ? '1 dot' : '3 dots'} to assign)
                  </h4>
        ...
        case 3: // Freebie Points / XP Summary
          ...
          if (newCharacter.faction === 'human') {
            const calculateFreebieSpent = () => {
              ...
              Object.entries(newCharacter.merits || {}).forEach(([meritId, qty]) => {
                const md = gameData.merits.find(m => m.merit_id === meritId);
                if (md?.special_notes?.includes('Does not increase cost of future merits')) {
                  flatTotal += 3; // Flat-cost merits always 3 XP (never free)
                } else if (md?.can_purchase_multiple === 'true') {
                  progressiveCount += qty;
                } else {
                  progressiveCount += 1;
                }
              });

              // Progressive merit total using closed-form sum
              // Human:     0 + 3 + 6 + ... + 3*(N-1) = 3 * N*(N-1)/2
              // Non-human: 3 + 6 + 9 + ... + 3*N     = 3 * N*(N+1)/2
              const N = progressiveCount;
              const progressiveTotal = isHuman
                ? 3 * N * (N - 1) / 2
                : 3 * N * (N + 1) / 2;

              return flatTotal + progressiveTotal;
            };
            ...
            return (
              <div className="space-y-6">
                ...
                <div className={`${themeClasses.card} p-3`}>
                  ...
                  <div className="space-y-2 max-h-80 overflow-y-auto">
                    {getAvailableMerits(newCharacter).map(merit => {
                      ...
                      const prereq = getMeritPrereqStatus(newCharacter, merit.merit_id);
                      ...
                      // Only count merits that actually use the free slot (flat-cost merits like Kinfolk/Delirium don't use it)
                      const isFlatCostMerit = (mid) => {
                        const md = gameData.merits.find(m => m.merit_id === mid);
                        return md?.special_notes?.includes('Does not increase cost of future merits');
                      };
                      const hasAnyFreeMeritSelected = meritKeys.some(mid => !isFlatCostMerit(mid));
                      ...
                      const showFreeMeritUI = isHuman && isCharacterCreation && !isFlatCostMerit(merit.merit_id);
                      const shouldGreyOut = showFreeMeritUI && hasAnyFreeMeritSelected && !hasMerit;
                      const isDisabledByFreeMerit = shouldGreyOut || !prereq.met;
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              {!prereq.met && prereq.label && (
                                <p className="text-xs text-red-400 mt-1">⛔ {prereq.label}</p>
                              )}
                            </div>
                                  ...
                                  <button
                                    onClick={() => {
                                      if (hasMerit) {
                                        const newMerits = { ...newCharacter.merits };
                                        delete newMerits[merit.merit_id];
                                        const extraFields = merit.merit_id === 'kinfolk' ? { kinfolkTribe: null } : {};
                                        setNewCharacter({ ...newCharacter, merits: newMerits, ...extraFields });
                ...
                {/* Kinfolk merit tribe selection */}
                {newCharacter.merits?.kinfolk && (
                  <div className={`${themeClasses.card} p-3`}>
                    <h4 className="font-bold mb-1">Kinfolk Tribe</h4>
                    <p className="text-gray-400 text-sm mb-3">Select the shifter tribe your character is related to. New characters with this merit begin play with Shifter Lore and their tribe's lore at no cost.</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {gameData.subfactions
                        .filter(tribe => tribe.faction_id === 'shifter' && (tribe.type === 'tribe' || tribe.type === 'fera') && tribe.subfaction_id !== 'black_spiral_dancer' && tribe.subfaction_id !== 'fallen_fera')
                        .map(tribe => {
                          const isSelected = newCharacter.kinfolkTribe === tribe.subfaction_id;
                          return (
                            <button
                              key={tribe.subfaction_id}
                              onClick={() => setNewCharacter({
                                ...newCharacter,
                                kinfolkTribe: isSelected ? null : tribe.subfaction_id
                              })}
                            ...
                            >
                              {tribe.subfaction_name}
                              {isSelected && <span className="ml-1 text-green-400">✓</span>}
                            </button>
                          );
                        })}
                    </div>
                    {!newCharacter.kinfolkTribe && (
                      <p className="text-yellow-400 text-xs mt-2">Select a tribe to receive your free lores at character creation.</p>
                    )}
                  </div>
                )}
              </div>
                  ...
                  {Object.entries(newCharacter.skills).length > 0 ? (
                    <div className="space-y-1 text-sm">
                      {Object.entries(newCharacter.skills).map(([skillId, level]) => {
                        const skill = gameData.skills?.find(s => s && s.skill_id === skillId);
                        return (
                ...
                <button
                  onClick={async () => {
                    ...
                    if (factionChangeCreationMode) {
                      ...
                      // Use cloud update function which handles state management
                      try {
                        await cloudUpdateCharacter(currentCharacterIndex, finalCharacter);
                        console.log('Faction change successfully saved to cloud:', finalCharacter.name);
                      } catch (error) {
                        console.warn('Failed to save faction change to cloud:', error);
                        // Fallback to direct state update if cloud fails
                        const newCharacters = [...characters];
                        newCharacters[currentCharacterIndex] = finalCharacter;
                        setCharacters(newCharacters);
                      }
                    } else {
                      // Normal character creation - use cloud create function
                      try {
                        // Clean the character data before sending to ensure it's properly serializable
                        const cleanCharacter = {
                          ...finalCharacter,
                          // Ensure all arrays are properly initialized
                          selfNerfs: (finalCharacter.selfNerfs || []).map(nerf => ({
                            id: Number(nerf.id),
                            name: String(nerf.name || ''),
                            type: String(nerf.type || ''),
                            category: String(nerf.category || ''),
                            description: String(nerf.description || ''),
                            source: String(nerf.source || '')
                          })),
                          // Ensure other arrays are clean
                          lores: finalCharacter.lores || [],
                          advancementHistory: finalCharacter.advancementHistory || [],
                          xpHistory: finalCharacter.xpHistory || [],
                          fundamentalPowers: finalCharacter.fundamentalPowers || [],
                          selectedPassions: finalCharacter.selectedPassions || [],
                          innateTreeIds: finalCharacter.innateTreeIds || [],
                          claimedInnateTreeIds: finalCharacter.claimedInnateTreeIds || [],
                          thornOptions: finalCharacter.thornOptions || [],
                          teachingsReceived: finalCharacter.teachingsReceived || [],
                          teachingsGiven: finalCharacter.teachingsGiven || [],
                          sharedWith: finalCharacter.sharedWith || [],
                          factionChanges: finalCharacter.factionChanges || [],
                          validationErrors: finalCharacter.validationErrors || [],
                          buildPlans: finalCharacter.buildPlans || []
                        };
                        
                        console.log('🚀 Character creation data being sent:', {
                          name: cleanCharacter.name,
                          faction: cleanCharacter.faction,
                          subfaction: cleanCharacter.subfaction,
                          breed: cleanCharacter.breed,
                          hasStats: !!cleanCharacter.stats,
                          hasSelfNerfs: cleanCharacter.selfNerfs.length > 0,
                          selfNerfsCount: cleanCharacter.selfNerfs.length,
                          selfNerfsData: cleanCharacter.selfNerfs,
                          serializable: JSON.stringify(cleanCharacter).length > 0
                        });
                        
                        await cloudCreateCharacter(cleanCharacter);
                        console.log('Character successfully saved to cloud:', cleanCharacter.name);
                      } catch (error) {
                        console.error('Failed to save character to cloud:', error);
                        console.error('Error details:', error.response?.data || error.message);
                        // Show specific error message to user
                        alert(`Failed to save character: ${error.response?.data?.message || error.message || 'Unknown error'}. Character will be saved locally instead.`);
                        // Fallback to direct state update if cloud fails
      ...
      <div className="w-full max-w-6xl mx-auto px-2 py-4 sm:px-4 sm:py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
          {filteredAndSortedCharacters.map((character) => (
            <div key={character._id || character.id} className={`${themeClasses.card} p-3 sm:p-4 lg:p-5 hover:shadow-lg transition-all`}>
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={() => handleDeleteCharacter(character._id || character.id)}
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  <span>Check-ins:</span>
                  <span>{character.checkInCount || 0}</span>
                </div>
              </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              <div key={character.id || character._id} className={`${themeClasses.card} p-5 md:col-span-2 lg:col-span-3`}>
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  ...
                  {character.faction === 'wraith' && character.shadowArchetype && (
                    <div className="md:col-span-2">
                      <label className={themeClasses.label}>Shadow</label>
                      <div className={themeClasses.input + ' flex items-center text-red-300'}>
                        {gameData.shadowArchetypes?.find(a => a.archetype_id === character.shadowArchetype)?.archetype_name || character.shadowArchetype}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className={`${themeClasses.card} p-3`}>
                ...
                {(() => {
                  const scorches = getScorchedBy(character);
                  if (!scorches.length) return null;
                  return (
                    <div className="mt-2">
                      <p className="text-xs text-gray-400">
                        <span className="font-semibold text-orange-400">Scorched by:</span> {scorches.join(', ')}
                      </p>
                    </div>
                  );
                })()}
                {character.faction === 'vampire' && (() => {
                  const curse = getClanCurse(character.subfaction);
                  if (!curse) return null;
                  return (
                    <div className="mt-2">
                      <p className="text-xs text-gray-400">
                        <span className="font-semibold text-red-400">Clan Curse:</span> {curse}
                      </p>
                    </div>
                  );
                })()}
              ...
              <div className={`${themeClasses.card} p-3 md:col-span-2 lg:col-span-3`}>
                <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-3 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                    ...
                    <div className="bg-white bg-opacity-20 rounded-lg p-3">
                      <div className="text-2xl font-bold text-yellow-200">{character.checkInCount || 0}</div>
                      <div className="text-blue-200 text-sm">Check-ins</div>
                    </div>
                    <div className="bg-white bg-opacity-20 rounded-lg p-3">
                      <div className="text-2xl font-bold text-green-200">{(character.checkInCount || 0) * 3}</div>
              ...
              <div className={`${themeClasses.card} p-3`}>
                ...
                <div className="space-y-3">
                  {getAvailableMerits(character, true)
                    ...
                    const canAdvanceNow = canAfford && prereq.met;
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            {!prereq.met && prereq.label && (
                              <p className="text-xs text-red-400 italic mt-1">⛔ {prereq.label}</p>
                            )}
                              ...
                              <div className="flex gap-2">
                                ...
                                {(!hasMerit || merit.can_purchase_multiple === 'true') && (
                                  <button
                                    ...
                                    disabled={!canAdvanceNow}
                                  >
                                    {canAdvanceNow ? 'Purchase' : !prereq.met ? 'Prerequisites Not Met' : canAfford ? 'Limit Reached' : 'Cannot Afford'}
                                  </button>
                                )}
                              </div>
                            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-5">
              ...
              <div className={`${themeClasses.card} p-3`}>
                ...
                <div className="max-h-[800px] overflow-y-auto space-y-4">
                  {/* Innate Trees - ensure ghouls display their three innate trees */}
                  {(() => {
                    const innateTreeIds = character.faction === 'human' && character.subfaction === 'ghoul' && (!character.innateTreeIds || !character.innateTreeIds.includes('celerity')) 
                      ? ['celerity', 'fortitude', 'potence'] 
                      : character.innateTreeIds || [];
                    
                    return innateTreeIds.length > 0 ? (
                      <div>
